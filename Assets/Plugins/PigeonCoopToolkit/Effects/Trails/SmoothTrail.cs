...
namespace PigeonCoopToolkit.Effects.Trails
{
    ...
    public class SmoothTrail : TrailRenderer_Base
    {
        ...
        */
        ...
        {
            //Profiler.BeginSample("UpdateTrail");
            ...
            int start = Mathf.Max(0, _controlPoints.Count - 3);
            int trailPointIndex = start * (PointsBetweenControlPoints + 1);
            for (int i = start; i < _controlPoints.Count; i++)
            {
            ...
            PCTrailPoint currentHandle = trail.Points[prevControlPointPointIndex];
            PCTrailPoint nextHandle = trail.Points[lastControlPointPointIndex];
            ...
            for (int pointBetween = 0; pointBetween < PointsBetweenControlPoints; pointBetween++)
            {
                ...
                if (TrailData.UseForwardOverride)
                {
                    trail.Points[prevControlPointPointIndex + pointBetween + 1].Forward = Vector3.Lerp(currentHandle.Forward, nextHandle.Forward, t);
                }
            }
            trail.Points[lastControlPointPointIndex].Position = _controlPoints[lastControlPointIndex].p;
            if (TrailData.UseForwardOverride)
            {
                trail.Points[lastControlPointPointIndex].Forward = currentHandle.Forward;
            }
            */
