...
namespace CLRSharp
{
    ...
    {
        static List<Stack<object>> sArrayList = new List<Stack<object>>();
        static Stack<object> sStack = new Stack<object>();
        static Stack<object> sRefObjs = new Stack<object>();
        static Stack<object> sRefFuncs = new Stack<object>();
        public static object[] GetArray(int i)
        {
            while (sArrayList.Count <= i)
            {
                sArrayList.Add(new Stack<object>());
            }

            if (sArrayList[i].Count > 0)
            {
                var arr = (object[])sArrayList[i].Pop();
                for (int j = 0; j < i; j++)
                {
                    arr[j] = null;
                }
        ...
        public static void ReleaseArray(object[] array)
        {
            if (array == null)
                return;

            while (sArrayList.Count <= array.Length)
            {
                sArrayList.Add(new Stack<object>());
            }

            sArrayList[array.Length].Push(array);
        }

        public static StackFrame GetStackFrame(string name, bool isStatic)
        {
            if (sStack.Count > 0)
            {
                return (StackFrame)sStack.Pop();
            }

            return new StackFrame(name, isStatic);
        }

        public static void ReleaseStackFrame(StackFrame stackFrame)
        {
            sStack.Push(stackFrame);
        }

        public static StackFrame.RefObj GetRefObj(StackFrame frame, int pos, StackFrame.RefType type)
        {
            if (sRefObjs.Count > 0)
            {
                var obj = (StackFrame.RefObj)sRefObjs.Pop();
                obj.frame = frame;
                obj.pos = pos;
                obj.type = type;
                obj._field = null;
            ...
            return new StackFrame.RefObj(frame, pos, type);
        }

        public static StackFrame.RefObj GetRefObj(IField field, object _this)
        {
            if (sRefObjs.Count > 0)
            {
                var obj = (StackFrame.RefObj)sRefObjs.Pop();
                obj.frame = null;
                obj.pos = 0;
                obj.type = StackFrame.RefType.field;
                obj._field = field;
                obj._this = _this;
                obj._array = null;
                return obj;
            ...
            return new StackFrame.RefObj(field, _this);
        }

        public static StackFrame.RefObj GetRefObj(Array array, int index)
        {
            if (sRefObjs.Count > 0)
            {
                var obj = (StackFrame.RefObj)sRefObjs.Pop();
                obj.frame = null;
                obj.pos = index;
                obj.type = StackFrame.RefType.Array;
                obj._field = null;
                obj._this = null;
                obj._array = array;
                return obj;
            ...
            return new StackFrame.RefObj(array, index);
        }

        public static void ReleaseRefObj(object obj)
        {
            sRefObjs.Push(obj);
        }

        public static RefFunc GetRefFunc(IMethod _method, object _this)
        {
            if (sRefFuncs.Count > 0)
            {
                var func = (RefFunc)sRefFuncs.Pop();
                func._method = _method;
                func._this = _this;
                return func;
            }

            return new RefFunc(_method, _this);
        }

        public static void ReleaseRefFunc(object obj)
        {
            sRefFuncs.Push(obj);
        }
    }
    ...
    public class ThreadContext
    {
        ...
        public string Dump()
        {
            string str = "";
            {
                // foreach(var s in GetStackFrames())
                var __enumerator1 = (GetStackFrames()).GetEnumerator();
                while (__enumerator1.MoveNext())
                {
                    var s = (StackFrame)__enumerator1.Current;
                    {
                        var pos = s.GetCode();
        ...
        public object ExecuteFunc(IMethod_Sharp method, object _this, object[] _params)
        {
            ...
            StackFrame stack = MemoryPool.GetStackFrame(method.Name, method.isStatic);//new StackFrame(method.Name, method.isStatic);
            stacks.Push(stack);

            for (int i = 0; i < method.ParamList.Count; i++)
            {
                if (method.ParamList[i].TypeForSystem == typeof(object) && _params[i] != null && _params[i].GetType().IsValueType)
                {
                    _params[i] = ValueOnStack.MakeVBox(_params[i], method.ParamList[i]);
                }
				// for enum
				else if (_params[i] != null && (method.ParamList[i].IsEnum() || _params[i] is Enum)) {
					_params[i] = Enum.ToObject(method.ParamList[i].TypeForSystem, _params[i]);
				}
            }
            bool _withpIsNew = false;
            object[] _withp = null;
            bool isctor = method.Name == ".ctor";
            if (isctor)
            {
                _withp = MemoryPool.GetArray(_params == null ? 1 : (_params.Length + 1));
                _withpIsNew = true;
                if (_params != null)
            ...
            if (_withpIsNew)
            {
                MemoryPool.ReleaseArray(_withp);
            }
            MemoryPool.ReleaseStackFrame(stack);
        ...
        bool JumpToErr(CodeBody body, StackFrame frame, Exception err)
        {
            ...
            int ehNearB = -1;
            {
                ...
                var __enumerator2 = (body.bodyNative.ExceptionHandlers).GetEnumerator();
                while (__enumerator2.MoveNext())
                {
                    var eh = __enumerator2.Current;
                    {
        ...
        void RunCode(StackFrame stack, CodeBody body)
        {
            while (true)
            {
                ...
                var _code = body.opCodes[stack._codepos];
                //if (_code.ToString().Contains("IL_0002") && _code.code == CodeEx.Isinst)
                //{
                //    environment.logger.Log(_code.ToString());
                //}
                //if (body.method.Name.Contains("CalculateStarLevel"))
                //{
                //    environment.logger.Log(_code.ToString());
                //}
//                 if (DebugLevel >= 9)
...
//                 }
                switch (_code.code)
                {
                    ...
                    case CodeEx.Ret:
                        ...
                        return;
                    case CodeEx.Leave:
                        stack.Leave(_code.tokenAddr_Index, body);
                        break;
                    case CodeEx.Leave_S:
                        stack.Leave(_code.tokenAddr_Index, body);
                        break;
