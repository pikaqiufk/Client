...
namespace BehaviourMachine {
    ...
    [NodeInfo(  category = "UnityGUI/Drawable/GUI/",
                ...
                url = "http://docs.unity3d.com/Documentation/ScriptReference/GUI.Button.html")]
    public class GUIButton : ConditionNode, IGUINode {
        ...
        public override Status Update () {
            ...
            if (guiStyle.isNone) {
                if (GUI.Button(this.GetRect(), guiContent)) {
                    // Send event?
                    if (onSuccess.id != 0)
                ...
                if (GUI.Button(this.GetRect(), guiContent, guiStyle.Value)) {
                    // Send event?
                    if (onSuccess.id != 0)
                        owner.root.SendEvent(onSuccess.id);
