		...
		static bool CreateBundleProc( string[] filelist, string target, bool isScene, BuildAssetBundleOptions option, BuildTarget platform, bool rebuild )
		{
			...
			if( !isScene )
			{
				...
				if( rebuild || CheckUpdateOfBundleAssets(filelist, target, option) )
				{
				    ...
				    var path = Path.GetDirectoryName(target) + "/" + target.Substring(("Assets/BundleAsset/").Length).Replace('/', '_');
                    //没有先创建文件夹
				    var dataPath = Application.dataPath;
                    var dir = Path.Combine(dataPath.Replace("Assets",string.Empty), Path.GetDirectoryName(path));
				    if (!Directory.Exists(dir))
				    {
				        Directory.CreateDirectory(dir);
				    }
                    
				    ret = BuildPipeline.BuildAssetBundle( null, list.ToArray(), path, option, platform );
				}
				else
				{
			...
			else
			{
				...
				{
                    var path = Path.GetDirectoryName(target) + "/" + target.Substring(("Assets/BundleAsset/").Length).Replace('/', '_');
                    string error = BuildPipeline.BuildStreamedSceneAssetBundle(filesAssets.ToArray(), path, platform);
		    ...
		    if (!ret)
		    {
               // throw new Exception("build assetbundle error!!" + filelist[0]);
		    }
