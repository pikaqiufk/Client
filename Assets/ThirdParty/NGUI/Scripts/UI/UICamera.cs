...
public class UICamera : MonoBehaviour
{
    ...
    static public bool IsPressed(GameObject go)
    {
        ...
        {
            // foreach(var touch in mTouches)
            var __enumerator1 = (mTouches).GetEnumerator();
            while (__enumerator1.MoveNext())
            {
                var touch = (KeyValuePair<int, MouseOrTouch>)__enumerator1.Current;
                if (touch.Value.pressed == go) return true;
            }
        }
    ...
    static public int touchCount
    {
        get
        {
            ...
            {
                // foreach(var touch in mTouches)
                var __enumerator2 = (mTouches).GetEnumerator();
                while (__enumerator2.MoveNext())
                {
                    var touch = (KeyValuePair<int, MouseOrTouch>)__enumerator2.Current;
    ...
    static public int dragCount
    {
        get
        {
            ...
            {
                // foreach(var touch in mTouches)
                var __enumerator3 = (mTouches).GetEnumerator();
                while (__enumerator3.MoveNext())
                {
                    var touch = (KeyValuePair<int, MouseOrTouch>)__enumerator3.Current;
    ...
    void OnApplicationPause()
    {
        ...
        if (useTouch)
        {
            ...
            {
                // foreach(var pair in mTouches)
                var __enumerator4 = (mTouches).GetEnumerator();
                while (__enumerator4.MoveNext())
                {
                    var pair = (KeyValuePair<int, MouseOrTouch>)__enumerator4.Current;
                    {
