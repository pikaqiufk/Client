...
public class UIPanel : UIRect
{
    ...
    [System.NonSerialized] public int skippedRenderQueue = 0;
    ...
    [SerializeField]
    bool childrenAlwaysVisible = false;
...
#endif
		{
			...
			for (int i = 0, imax = list.Count; i < imax; ++i)
			{
				...
				if (p.renderQueue == RenderQueue.Automatic)
				{
					...
					p.UpdateDrawCalls();
                    rq += p.drawCalls.Count + p.skippedRenderQueue;
				}
				else if (p.renderQueue == RenderQueue.StartAt)
				{
					p.UpdateDrawCalls();
					if (p.drawCalls.Count != 0)
						rq = Mathf.Max(rq, p.startingRenderQueue + p.drawCalls.Count + p.skippedRenderQueue);
				}
				else // Explicit
				{
					p.UpdateDrawCalls();
	...
	void FillAllDrawCalls ()
	{
		...
		for (int i = 0; i < widgets.Count; ++i)
		{
            ...
            if (w.SkippedRenderQueue > 0 && dc != null)
            {
                dc.SkippedRenderQueue += w.SkippedRenderQueue;
                skippedRenderQueue += w.SkippedRenderQueue;
	            mat = null;
            }
		}
	...
	void UpdateDrawCalls ()
	{
		...
		for (int i = 0; i < drawCalls.Count; ++i)
		{
			...
			dc.renderQueue = (renderQueue == RenderQueue.Explicit) ? startingRenderQueue : startingRenderQueue + skip + i;

			skip += dc.SkippedRenderQueue;
    ...
    public virtual Vector3 CalculateConstrainOffset(Vector2 min, Vector2 max, bool halfSoft = false)
	{
		...
		if (softBorderPadding && clipping == UIDrawCall.Clipping.SoftClip)
		{
            if (halfSoft)
            {
                var x = mClipSoftness.x/2;
                var y = mClipSoftness.y/2;
                minArea.x += x;
                minArea.y += y;
                maxArea.x -= x;
                maxArea.y -= y;       
