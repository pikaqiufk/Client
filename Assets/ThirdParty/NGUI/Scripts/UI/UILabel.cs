...
public class UILabel : UIWidget
{
	...
	//-1，如果不被属性绑定控制就会被工具刷
	//其余的都是字典ID
    [HideInInspector][SerializeField] int mDictionaryId = -2;

    public int DictionaryId
    {
        get { return mDictionaryId; }
        set
        {
            if (mDictionaryId == value) return;
            mDictionaryId = value;
            InitDictionary();
        }
    }
    ...
    public void InitDictionary()
    {
        if (mDictionaryId > 0)
        {
            var tbDic = Table.GetDictionary(mDictionaryId);
	        if (tbDic != null)
	        {
		        text = tbDic.Desc[0];
	        }
	        ...
	        {
				//var log = string.Format("UILabel[{0}] DictionaryId[{1}] can't find", gameObject.transform.FullPath(), mDictionaryId);
