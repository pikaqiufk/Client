...
public class UISpriteAnimation : MonoBehaviour
{
		...
		if (mActive && mSpriteNames.Count > 1 && Application.isPlaying && mFPS > 0)
		{
			...
			if (rate < mDelta)
			{
				...
				if (mActive)
				{
                    ...
                    mSprite.transform.localScale = mScale;
				}
			}
		}
	}
	...
	public void RebuildSpriteList ()
	{
	    ...
	    mScale = mSprite.transform.localScale;
		mSpriteNames.Clear();
		...
		if (mSprite != null && mSprite.atlas != null)
		{
			...
			mSpriteNames.Sort((a, b) =>
			{
                try
                {
                    var ia = int.Parse(a.Substring(mPrefix.Length));
                    var ib = int.Parse(b.Substring(mPrefix.Length));
                    return ia - ib;
                }
                catch
                {
