...
public class UIInput : MonoBehaviour
{
	...
	public string Validate (string val)
	{
        ...
        if (characterLimit > 0 && getStringLength(val) > characterLimit)
...
//             return;
		...
		for (int i = 0, imax = text.Length; i < imax; ++i)
		{
            ...
            if (characterLimit > 0 && getStringLength(left + right + text + right) > characterLimit) break;
    ...
    protected void ExecuteOnHide()
    {
        if (current == null && EventDelegate.IsValid(onHide))
        {
            current = this;
