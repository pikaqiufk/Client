...
static public class NGUITools
{
    ...
    static public void UpdateWidgetCollider(BoxCollider box, bool considerInactive, float left, float right, float top, float bottom)
    {
        if (box != null)
        {
            GameObject go = box.gameObject;
            ...
            box.center = new Vector3(box.center.x - left + right, box.center.y + top - bottom,0);
            box.size = new Vector3(box.size.x + left + right, box.size.y + top + bottom,0);
    ...
    static public UIPanel CreateUI(Transform trans, bool advanced3D, int layer)
    {
        ...
        {
            {
                var __list1 = UIRoot.list;
                var __listCount1 = __list1.Count;
                for (int __i1 = 0; __i1 < __listCount1; ++__i1)
                {
                    var r = (UIRoot)__list1[__i1];
                    {
    ...
    static public void MarkParentAsChanged(Transform go)
    {
        UIRect[] rects = go.GetComponentsInChildren<UIRect>();
        for (int i = 0, imax = rects.Length; i < imax; ++i)
            rects[i].ParentHasChanged();
