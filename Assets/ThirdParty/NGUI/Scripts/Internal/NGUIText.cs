...
static public class NGUIText
{
	...
	/// Text wrapping functionality. The 'width' and 'height' should be in pixels.
	/// </summary>
    static int offsetSpaceIdx = 0;

    private static bool mTest = true;
    static public bool WrapText(string text, out string finalText, bool keepCharCount, bool isOnlyFirst = false)
    {
        offsetSpaceIdx = 0;
		...
		for (; offset < textLength; ++offset)
		{
		    ...
		    //ushort x = ch;
            //Logger.Error("------{0}---{1:X}", ch, x);
            //0000-024F 基本拉丁文 扩展
            //1E00-1EFF：拉丁文扩充附加 
            if (ch < 591 || (ch >= 7680 && ch <= 7935))
		    {
                if (eastern == true)
                {
                    offsetSpaceIdx = offset;
                }
                eastern = false;
		    }
            else
...
// 		   			// New line character -- start a new line
			...
			if (Mathf.RoundToInt(remainingWidth) < 0)
			{
                if (eastern == false)
			    {
                    if ((offset < (textLength - 1) && offsetSpaceIdx > start))
                    {
                        if (!IsSpace(text[offset + 1]))
                        {
                            offset = offsetSpaceIdx;
                        }
                        else
                        {
                            offset++;
