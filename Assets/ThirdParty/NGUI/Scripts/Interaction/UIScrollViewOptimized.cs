...
public class UIScrollViewOptimized : MonoBehaviour
{
    static public BetterList<UIScrollViewOptimized> list = new BetterList<UIScrollViewOptimized>();

    public LinkedList<UIWidget> mChildren = new LinkedList<UIWidget>(); 
...
//             }
    ...
    {
        if (HasNextWidget(LastIndex + 1) && HasNextWidget(FirstIndex - 1))
        {
            return false;
        }
    ...
    public virtual void UpdateScrollbars(bool recalculateBounds)
    {
        ...
        var count = widgetProvider.Count();
        var length = count - (LastIndex - FirstIndex) - 1;
        if (count == 0 || length == 0)
        {
        ...
        else
        {
            ...
            verticalScrollBar.value = ((float)FirstIndex) / length;
            UIScrollBar sb = verticalScrollBar as UIScrollBar;
            if (sb != null) sb.barSize = ((float)scrollBarSize) / count;
        }
    }
...
//         }
    ...
    public virtual void MoveRelative(Vector3 relative)
    {
        ...
        UpdateCell(relative.y > 0);

        if (centerOnChild)
        {
            centerOnChild.DoCellScale();
        }
    ...
    private bool needNextWidget = false;
    private void UpdateCell(bool up)
    {
        if (mPanel == null)
        {
            return;
        }
        // 如果是向上移动，那第一个格子看不见的时候，把这个格子删掉
        if (up)
        {
            if (HasNextWidget(LastIndex + 1))
            {
                if (mChildren.Count > 0 && !mPanel.IsVisible(mChildren.First.Value))
                {
                    NGUITools.Destroy(mChildren.First.Value.cachedGameObject);
                    BeforeDestroy(mChildren.First.Value);
                    mChildren.RemoveFirst();

                    ++FirstIndex;
                }

                // add new one.
                while ((mChildren.Count == 0 || mPanel.IsVisible(mChildren.Last.Value)) && HasNextWidget(LastIndex + 1))
                {
                    var widget = GetNextWidget(LastIndex + 1);
                    if (widget != null)
                    {
                        ++LastIndex;

                        widget.cachedTransform.parent = mTrans;
                        widget.cachedTransform.localScale = Vector3.one;
                        var pos = Vector3.zero;
                        if (mChildren.Count > 0)
                        {
                            pos.y = mChildren.Last.Value.cachedTransform.localPosition.y -
                                    mChildren.Last.Value.height - padding;
                        }
                        else
                        {
                            pos.y = mPanel.GetViewSize().y/2;
                        }
                        pos.x = -mPanel.GetViewSize().x / 2;
        ...
        {
            if (HasNextWidget(FirstIndex - 1))
            {
                if (mChildren.Count > 0 && !mPanel.IsVisible(mChildren.Last.Value))
                {
                    NGUITools.Destroy(mChildren.Last.Value.cachedGameObject);
                    BeforeDestroy(mChildren.Last.Value);
                    mChildren.RemoveLast();

                    --LastIndex;
                }

                while ((mChildren.Count == 0 || mPanel.IsVisible(mChildren.First.Value)) && HasNextWidget(FirstIndex - 1))
                {
                    if (FirstIndex - 1 == 0)
                    {
                        needNextWidget = true;
                    ...
                    // add new one.
                    var widget = GetNextWidget(FirstIndex - 1);
                    if (widget != null)
                    {
                        ...
                        if (mChildren.Count > 0)
                        {
                            pos.y = mChildren.First.Value.cachedTransform.localPosition.y +
                                    widget.height + padding;
                        }
                        else
                        {
                            pos.y = -mPanel.GetViewSize().y / 2 + widget.height;
                        }
                        pos.x = -mPanel.GetViewSize().x / 2;
                        widget.cachedTransform.localPosition = pos;
    ...
    public void UpdateCell(UIWidget widget)
    {
        var node = mChildren.First;
        var nextYPos = 0.0f;
        ...
        while (node.Next != null)
        {
            if (needAdjust)
            {
                var p = node.Value.cachedTransform.localPosition;
                p.y = nextYPos;
                node.Value.cachedTransform.localPosition = p;
                nextYPos -= node.Value.height;
                nextYPos -= padding;
            }

            if (node.Value == widget)
            {
                nextYPos = node.Value.cachedTransform.localPosition.y - node.Value.height - padding;
                needAdjust = true;
            }
...
#endif
    public IUIScrollViewOptimizedWidgetProvider widgetProvider;
    private UIWidget GetNextWidget(int index)
    {
        return widgetProvider.GetWidget( index);
    }
    private bool HasNextWidget(int index)
    {
        return widgetProvider.HasNextWidget(index);
    }
    private void BeforeDestroy(UIWidget widget)
    {
        if (widget)
        {
            widgetProvider.BeforeDestroy(widget);    
        }
    }
    ...
    /// 重置scrollview
    /// </summary>
    /// <param name="dir">true 为向下刷新，false为向上刷新</param>
    public void Reset(int index = 0, bool dir = false)
    {
        foreach (var child in mChildren)
        {
            BeforeDestroy(child);
            NGUITools.Destroy(child.cachedGameObject);
        }

        if (mPanel)
        {
            mTrans.localPosition = originalLocalPosition;
    ...
    /// </summary>
    /// <param name="dir">true 为向下刷新，false为向上刷新</param>
    public void NofifyUpdate(bool dir = false)
    {
        UpdateCell(dir);
    }
    ...
    {
        if (mChildren.Count > 0 && mPanel)
        {
            if (bounds.size.y < mPanel.GetViewSize().y)
            {
                var offset = mPanel.GetViewSize().y / 2 + mPanel.clipOffset.y - bounds.max.y;
                MoveRelative(new Vector3(0, offset, 0));
            }
            else
            {
                // 如果当前最后一个已经被加载（知道高度），就直接计算并拉到最下面
                // 否则就Reset到最下面
                if (index == LastIndex)
                {
                    var offset = -mPanel.GetViewSize().y/2 + mPanel.clipOffset.y - bounds.min.y;
                    MoveRelative(new Vector3(0, offset, 0));
                }
                else
                {
    ...
    private void RequestUpperWidget()
    {
        widgetProvider.RequestUpperWidget();
    }
