...
[AddComponentMenu("NGUI/Interaction/Center Simple Scroll View on Child")]
public class UICenterOnChildSimple : MonoBehaviour
{
	...
	UIScrollViewSimple mScrollView;
...
// #endif
// 			{
//                 SpringPanelMoveBy.Begin(mScrollView.panel.cachedGameObject, -localOffset, springStrength).onFinished = onFinished;
// 				//SpringPanel.Begin(mScrollView.panel.cachedGameObject,panelTrans.localPosition - localOffset, springStrength).onFinished = onFinished;
// 			}
// 		}
...
// 	}
    ...
    {
        if (mScrollView.movement == UIScrollViewSimple.Movement.Horizontal)
        {
            var pos = mScrollView.transform.localPosition;

            var offset = pos.x;
            var grid = mScrollView.thisGrid;
            var cellSize = grid.cellWidth;
            var relative = new Vector3();
            if (offset < 0)
            {
                if (offset > -cellSize/2.0f)
                {
                    relative = new Vector3(-offset,0,0);
                    SpringPanelMoveBy.Begin(mScrollView.panel.gameObject, relative, 13f, onFinished).strength = springStrength;
                }
                else
                {
                    relative = new Vector3(-grid.cellWidth - offset, 0, 0);
                    SpringPanelMoveBy.Begin(mScrollView.panel.gameObject, relative, 13f, onFinished).strength = springStrength;        
                }
            }
            else
