...
public class UIScrollView : MonoBehaviour
{
    ...
    /// <summary>
    /// Whether to reset panel offset when OnEnable() called!
    /// </summary>	
	...
	void OnEnable ()
	{
        ...
        if (resetOffsetOnEnable)
        {
            mMomentum = Vector3.zero;
            DisableSpring();
            mTrans.localPosition += new Vector3(panel.clipOffset.x, panel.clipOffset.y, 0);
	...
	public virtual void UpdateScrollbars (bool recalculateBounds)
	{
            ...
            || UndargEnd != null)
	    {
	        ...
	        {
                intViewSize = Mathf.RoundToInt(clip.z);
                if (mPanel.clipping == UIDrawCall.Clipping.SoftClip)
                {
                    softness = mPanel.clipSoftness.x;
                }
                clipMove = clip.x;
	        ...
	        {
                intViewSize = Mathf.RoundToInt(clip.w);
                if (mPanel.clipping == UIDrawCall.Clipping.SoftClip)
                {
                    softness = mPanel.clipSoftness.y;
                }
                clipMove = clip.y;
            ...
            halfViewSize -= softness;
            contentMin -= softness;
            contentMax -= softness;

            float viewSize = halfViewSize * 2f;

            float viewMin = clipMove - halfViewSize;
            float viewMax = clipMove + halfViewSize;


            if (viewMax - viewMin > contentMax - contentMin)
	        {
	            if (DargBegin)
	            {
