...
#region Common functionality
	...
	[System.NonSerialized] protected UIDragScrollViewSimple mDragScrollViewSimple = null;
	...
	protected virtual void Start ()
	{
        ...
        mDragScrollViewSimple = GetComponent<UIDragScrollViewSimple>();
	}
	...
	protected virtual void OnDragStart ()
	{
		...
		{
            if (restriction == Restriction.LeftTopRightButtom)
		    {
                Vector2 delta = mTouch.totalDelta;

                //Logger.Error("delta {0}", delta);

                if (Mathf.Abs(delta.x) < 0.1f && Mathf.Abs(delta.y) > 1.0)
		        {//垂直可以拖动
                    StartDragging();   
                    return;
		        }
            ...
            else if (restriction == Restriction.RightTopLeftButtom)
		    {
                Vector2 delta = mTouch.totalDelta;
                if (Mathf.Abs(delta.x) < 0.1f && Mathf.Abs(delta.y) > 1.0)
                {//垂直可以拖动
			...
			else if (restriction == Restriction.Horizontal)
			{
				Vector2 delta = mTouch.totalDelta;
	...
	protected virtual void OnDragDropStart ()
	{
        ...
        if (mDragScrollViewSimple) mDragScrollViewSimple.enabled = false;
	...
	protected IEnumerator EnableDragScrollView ()
	{
        ...
        if (mDragScrollViewSimple) mDragScrollViewSimple.enabled = true;
	}
}
