...
public class UIProgressBar : UIWidgetContainer
{
		...
		set
		{
			...
			if (mValue != val)
			{
				...
				if (before != this.value)
				{
                    ...
                    //使绑定的进度条在没有激活的情况下也随着数据变化，一些进度条是先设置再显示
	...
	public virtual void ForceUpdate ()
	{
		...
		if (mFG != null)
		{
            ...
            if (sprite != null && sprite.type == UIBasicSprite.Type.Advanced)
		    {
                switch (mFill)
                {
                    case FillDirection.LeftToRight:
                        if (Mathf.Abs(value - 1.0f) < 0.001)
                        {
                            sprite.rightType  = UIBasicSprite.AdvancedType.Sliced;
                        }
                        else
                        {
                            sprite.rightType = UIBasicSprite.AdvancedType.Invisible;
                        }
                        break;
                    case FillDirection.RightToLeft:
                        if (Mathf.Abs(value - 1.0f) < 0.001)
                        {
                            sprite.leftType = UIBasicSprite.AdvancedType.Sliced;
                        }
                        else
                        {
                            sprite.leftType = UIBasicSprite.AdvancedType.Invisible;
                        }
                        break;
                    case FillDirection.BottomToTop:
                        if (Mathf.Abs(value - 1.0f) < 0.001)
                        {
                            sprite.topType = UIBasicSprite.AdvancedType.Sliced;
                        }
                        else
                        {
                            sprite.topType = UIBasicSprite.AdvancedType.Invisible;
                        }
                        break;
                    case FillDirection.TopToBottom:
                        if (Mathf.Abs(value - 1.0f) < 0.001)
                        {
                            sprite.bottomType = UIBasicSprite.AdvancedType.Sliced;
                        }
                        else
                        {
                            sprite.bottomType = UIBasicSprite.AdvancedType.Invisible;
                        }
                        break;
                    default:
                        break;
