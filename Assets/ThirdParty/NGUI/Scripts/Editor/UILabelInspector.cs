...
public class UILabelInspector : UIWidgetInspector
{
	...
	protected override bool ShouldDrawProperties ()
	{
		...
		EditorGUI.BeginDisabledGroup(!isValid);
		{
            ...
            SerializedProperty spDic = serializedObject.FindProperty("mDictionaryId");
            GUILayout.BeginHorizontal();
            GUILayout.BeginVertical(GUILayout.Width(76f));
            GUILayout.Space(3f);
            ...
            Rect rectDic = EditorGUILayout.GetControlRect(GUILayout.Height(20.0f));

            GUI.changed = false;
            var textDic = EditorGUI.IntField(rectDic, spDic.intValue);
		    if (GUI.changed)
		    {
                if (textDic > 0)
                {
                    var str = GetDictionaryString(textDic);
                    sp.stringValue = str;    
		        }
                spDic.intValue = textDic;
		    }
