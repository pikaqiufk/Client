...
public class UIPrefabTool : EditorWindow
{
    ...
    public void Reset()
    {
        {
            // foreach(var item in mItems)
            var __enumerator2 = (mItems).GetEnumerator();
            while (__enumerator2.MoveNext())
            {
                var item = (Item)__enumerator2.Current;
                DestroyTexture(item);
            }
        }
        mItems.Clear();
    ...
    void OnGUI()
    {
        ...
        if (mReset && type == EventType.Repaint)
        {
            ...
            {
                // foreach(var item in mItems)
                var __enumerator8 = (mItems).GetEnumerator();
                while (__enumerator8.MoveNext())
                {
                    var item = (Item)__enumerator8.Current;
