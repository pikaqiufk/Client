...
public class UIAtlasMaker : EditorWindow
{
    ...
    List<Texture> GetSelectedTextures()
    {
        ...
        if (Selection.objects != null && Selection.objects.Length > 0)
        {
            ...
            {
                var __array1 = objects;
                var __arrayLength1 = __array1.Length;
                for (int __i1 = 0; __i1 < __arrayLength1; ++__i1)
                {
                    var o = (Object)__array1[__i1];
                    {
    ...
    static List<Texture2D> LoadTextures(List<Texture> textures)
    {
        ...
        {
            var __list2 = textures;
            var __listCount2 = __list2.Count;
            for (int __i2 = 0; __i2 < __listCount2; ++__i2)
            {
                var tex = (Texture)__list2[__i2];
                {
    ...
    Dictionary<string, int> GetSpriteList(List<Texture> textures)
    {
        ...
        if (NGUISettings.atlas != null)
        {
            BetterList<string> spriteNames = NGUISettings.atlas.GetListOfSprites();
            {
                ...
                var __enumerator5 = (spriteNames).GetEnumerator();
                while (__enumerator5.MoveNext())
                {
                    var sp = (string)__enumerator5.Current;
                    {
    ...
    {
        {
            ...
            {
                var sp = (UISpriteData)__list6[__i6];
                {
                    if (sp.name == se.name)
                    {
                        sp.CopyFrom(se);
    ...
    {
        {
            ...
            {
                var se = (SpriteEntry)__list8[__i8];
                se.Release();
            }
        }
        ...
        if (spriteList.Count > 0)
        {
            NGUIEditorTools.DrawHeader("Sprites", true);
            {
                ...
                int index = 0;
                {
                    ...
                    var __enumerator10 = (spriteList).GetEnumerator();
                    while (__enumerator10.MoveNext())
                    {
                        var iter = (KeyValuePair<string, int>)__enumerator10.Current;
                        {
                            ++index;
