...
namespace Thinksquirrel.Utilities
{
    ...
    [CameraShakeDocumentationName("Thinksquirrel.Utilities.CameraShake")]
    public class CameraShake : CameraShakeBase
    {
        ...
        public Vector3 shakePositionOffset { get; private set; }
        ...
        public void Shake()
        {
            ...
            for (int i = 0, camerasCount = cameras.Count; i < camerasCount; i++)
            {
                ...
                this.StartCoroutine(DoShake_Internal(cam, seed, shakeType, numberOfShakes, shakeAmount,
                    rotationAmount, distance, speed, decay, uiShakeModifier, multiplyByTimeScale, null));
            }
        }
        ...
        public void Shake(ShakeType shakeType, int numberOfShakes, Vector3 shakeAmount, Vector3 rotationAmount, float distance, float speed, float decay, float uiShakeModifier, bool multiplyByTimeScale)
        {
            ...
            {
                Camera cam = cameras[i];
                this.StartCoroutine(DoShake_Internal(cam, seed, shakeType, numberOfShakes, shakeAmount, rotationAmount, distance, speed, decay, uiShakeModifier, multiplyByTimeScale, null));
            }
        }
        ...
        public void Shake(System.Action callback)
        {
            ...
            {
                Camera cam = cameras[i];
                this.StartCoroutine(DoShake_Internal(cam, seed, shakeType, numberOfShakes, shakeAmount, rotationAmount, distance, speed, decay, uiShakeModifier, multiplyByTimeScale, callback));
            }
        }
        ...
        public void CancelShake(float time)
        {
            ...
            {
                StopAllCoroutines();
                this.StartCoroutine(DoResetState(cameras, m_ShakeCount, time));
            }
        }
            ...
            while (currentShakes > 0)
            {
                ...
                switch (state._shakeType)
                {
                    ...
                    case ShakeType.LocalPosition:
                        //cachedTransform.localPosition = camOffset + cachedTransform.localPosition;
                        //cachedTransform.localRotation = Quaternion.Euler(cachedTransform.localRotation.eulerAngles + camRot.eulerAngles);
                        shakePositionOffset = camOffset;
                        shakeRotationOffset = camRot;
                        break;
                }
