...
// 			encoder.VBR = true;
		...
		private byte[] EncodeSpeech( short[] rawData, BandMode mode )
		{
			var encoder = m_narrow_enc;

            
            encoder.Quality = 4;
            encoder.VBR = false;

			int padSize = (rawData.Length+encoder.FrameSize-1)/encoder.FrameSize*encoder.FrameSize;
			int pad = padSize - rawData.Length;
			short[] padData = new short[padSize];
			System.Array.Copy(rawData,padData,rawData.Length);
			byte[] encodedData = new byte[rawData.Length*2];
			try
			{
				int encodedSize = encoder.Encode( padData, 0, padData.Length, encodedData, 0, encodedData.Length );
				byte[] outData = new byte[encodedSize+8];
				System.Array.Copy(encodedData, 0, outData, 8, encodedSize);
				byte[] sampleCount = BitConverter.GetBytes(rawData.Length);
				byte[] padCount    = BitConverter.GetBytes(pad);
				System.Array.Copy(sampleCount, 0, outData, 0, 4 );
				System.Array.Copy(padCount, 0, outData, 4, 4 );
				return outData;
			}
			catch( Exception e )
			{
		...
		private short[] DecodeSpeech( byte[] rawData, BandMode mode )
		{ 
			var decoder = m_narrow_dec;
			int sampleCount = BitConverter.ToInt32( rawData, 0 );
			int padCount = BitConverter.ToInt32( rawData, 4 );
			short[] decodeData = new short[sampleCount+padCount];
			int decodedCount = decoder.Decode( rawData, 8, rawData.Length-8, decodeData, 0, false );
			if( decodedCount != 0 )
			{
				short[] outData = new short[sampleCount];
				System.Array.Copy(decodeData, outData, sampleCount);
				return outData;
			}
			else
			{
				Debug.Log("u3d decode error count="+decodedCount);
				return new short[] { 0 };
			}
		}
	}
