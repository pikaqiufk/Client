...
#if !UNITY_3_5
[CustomPropertyDrawer(typeof(TargetBindingProperty))]
public class TargetBindingPropertyDrawer : PropertyDrawer
#else
        ...
        for (int i = 0, imax = comps.Length; i < imax; ++i)
        {
            ...
            for (int b = 0; b < fields.Length; ++b)
            {
                ...
                if (filter != typeof(void))
                {
                    if (canConvert)
                    {
                        if (!TargetBindingProperty.Convert(field.FieldType, filter)) continue;
                    }
            ...
            for (int b = 0; b < props.Length; ++b)
            {
                ...
                if (filter != typeof(void))
                {
                    if (canConvert)
                    {
                        if (!TargetBindingProperty.Convert(prop.PropertyType, filter)) continue;
                    }
